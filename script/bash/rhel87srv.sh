...
#!/bin/sh
## Loading Variables
IP_ADDR=192.168.52.130
HOSTNAME=rhel87srv0.rhel87srv0.ca
SHORTNAME=rhel87srv0
DOMAIN=rhel87srv0.ca
REALM=RHEL87SRV0.CA
LDAPHOME=/home/ldap
...
# Installing everything unattended | --forwarder=8.8.8.8
ipa-server-install --domain=$DOMAIN --realm=$REALM --ds-password=password --admin-password=password --hostname=$HOSTNAME --ip-address=$IP_ADDR --reverse-zone=52.168.192.in-addr.arpa. --no-forwarders --allow-zone-overlap --setup-dns --unattended
...
# Fixing resolv.conf
# sed -i 's/nameserver 127.0.0.1/nameserver 192.168.52.XXX/' /etc/resolv.conf
...
# nmcli connection modify eth0 ipv4.dns 192.168.52.130
...
echo "[base]" >> /var/www/html/repos/base.repo
echo "name = Base Reporitory for RHEL 8" >> /var/www/html/repos/base.repo
echo "baseurl = http://rhel87srv0.rhel87srv0.ca/repos/rhel8/BaseOS" >> /var/www/html/repos/base.repo
echo "enabled = 0" >> /var/www/html/repos/base.repo
echo "gpgcheck = 0" >> /var/www/html/repos/base.repo

echo "[appstream]" >> /var/www/html/repos/base.repo
echo "name = App Stream Reporitory for RHEL 8" >> /var/www/html/repos/base.repo
echo "baseurl = http://rhel87srv0.rhel87srv0.ca/repos/rhel8/AppStream" >> /var/www/html/repos/base.repo
echo "enabled = 0" >> /var/www/html/repos/base.repo
echo "gpgcheck = 0" >> /var/www/html/repos/base.repo
